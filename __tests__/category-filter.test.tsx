...
describe("CategoryFilter", () => {
  ...
  it("highlights the selected category", () => {
    const { rerender } = render(<CategoryFilter selectedCategory="dinner" onCategoryChange={mockOnCategoryChange} />)

    const dinnerChip = screen.getByText(/Dinner/i).closest(".MuiChip-root")
    expect(dinnerChip).toHaveStyle({ backgroundColor: "#ff6b35" })

    rerender(<CategoryFilter selectedCategory="breakfast" onCategoryChange={mockOnCategoryChange} />)

    const breakfastChip = screen.getByText(/Breakfast/i).closest(".MuiChip-root")
